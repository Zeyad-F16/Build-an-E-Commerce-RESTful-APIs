...
exports.createProductValidator = [
  ...
  check('subCategory')
    ...
    .then((idResult)=>{
      if(idResult.length < 1 || idResult.length !== subCategoryId.length){
        return Promise.reject(new Error(`no subCategory for this Id ${subCategoryId}`));
      }
    })
    )
    .custom((value,{req}) =>
      subCategoryModel.find({category : req.body.category})
      .then((subCategories)=>{
       ...
       if(!checker(value,subCategoryIdInDB)){
         return Promise.reject(new Error('subCategory not belong to this category'));
       }
      })
    ),
